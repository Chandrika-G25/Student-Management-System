...
app = Flask(__name__, 
            ...
            static_url_path="")
app.config["JWT_SECRET_KEY"] = os.environ.get("JWT_SECRET_KEY", "student_management_secret_key_change_me")
...
# Auto-initialize database tables and default admin on startup if enabled
if os.environ.get("AUTO_INIT_DB", "true").lower() in ("true", "1", "yes"):
    try:
        setup_database()
    except Exception as e:
        print("Note: Auto-init database deferred or encountered an error:", e)
...
@app.route("/api/setup-db", methods=["GET", "POST"])
def trigger_setup_db():
...
def get_attendance():
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
        SELECT a.id, a.student_id, s.student_id as roll_no, s.full_name, a.attendance_date, a.status
        FROM attendance a
